...
def create_graph(final_data,award_data):
    ...
    movies_length = len(final_data) #n
    contributors_length = len(influence_score) #m
    ...
    for row in final_data:
        ...
        for i in range(len(contributors)):
            if len(contributors[i].strip()) > 0:    
                adj_matrix[contributors[i]] =
    
    hits_algo(adj_matrix,influence_score)
